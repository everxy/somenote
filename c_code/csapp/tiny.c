/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 五  5/ 5 15:47:22 2017
 ************************************************************************/
...
void doit(int fd)
{
    ...
    if(stat(filename,&sbuf) < 0) {
        clienterror(fd,filename,"404","not found","tiny can't find this file");
        return ;
    }

    if(is_static) {
        if(!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
            clienterror(fd,filename,"403","forbidden","tiny can't read");
            return ;
        }
...
void serve_static(int fd,char *filename,int filesize)
{
    ...
    /*将srcfd的钱filesize个字节映射到一个从地址srcp开始的私有只读虚拟内存区域
    * 映射完成后不需要此文件描述符，所以关闭
    */ 
