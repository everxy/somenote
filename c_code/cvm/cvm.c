...
const char *fake = "/var/cos/cvm/sys/.fake";
const char *virtio = "/var/cos/cvm/iso/virtio_tiny.iso";
const char *vnctoken = "/var/cos/cvm/vnc_tokens";
...
//	} else {
//		sprintf(path, "%s/CLOS%d", cldir, id);
//	}
...
//VM in viewMode or not
bool viewMode(int id) {
    char buf[1024];
    char tmp[10];
    sprintf(tmp,"VM%d:", id);
    FILE *fp;
...
bool vmdel(int id) {
    ...
    xmlog(cmd);
    system(cmd);
    sprintf(cmd, "sed -i '/VM%d:/d' %s", id, vnctoken);
    xmlog(cmd);
    system(cmd);
	if(fileExist(file)) {
		return false;
...
bool vmon(int id, int cpu, char *cpu_type, int mem, char *mac, int type=0, char *br=NULL) {
	if(vmison(id)) {
		return true;
	}
	sprintf(file, "%s/VM%d", vmdir, id);
	sprintf(cmd, "qemu-kvm -name VM%d -smp %d,maxcpus=240 -cpu %s -m %d -rtc base=localtime -drive file=%s,if=virtio,media=disk,index=0 -vnc :%d -net nic,macaddr=%s,model=virtio -net tap,script=%s,downscript=%s,ifname=VM%d -usb -usbdevice tablet -enable-kvm -chardev socket,id=CL%d,path=%s/CL%d,server,nowait -monitor chardev:CL%d -chardev socket,id=QM%d,path=%s/QM%d,server,nowait -qmp chardev:QM%d > /dev/null 2>&1 &", id, cpu, cpu_type, mem, file, id+100, mac, ifup, ifdown, id, id, cldir, id, id, id, qmdir, id, id);
	xmlog(cmd);
//	printf("%s\n", cmd);
//	system(cmd);
	exec(cmd);
...
*/
		...
		monitor(id, "c", result);
        if( viewMode(id) ) {
            monitor(id, "change vnc password cac028", result);
        }
		return true;
	} else{
		return false;
 ...
 *
 */
bool oson(int id,int cpu, char *cpu_type ,int mem,char *mac,char *os_iso) {
    if(vmison(id)) {
	return false;
    }
    sprintf(file, "%s/BACK%d", vmdir, id);
//    printf("A:%s",file);
    sprintf(cmd, "qemu-kvm -name VM%d -smp %d,maxcpus=240 -cpu %s -m %d -drive file=%s,if=ide,media=disk,index=0 -drive file=%s,if=virtio,media=disk,index=1 -drive file=%s,if=ide,media=cdrom -cdrom %s/%s -vnc :%d -enable-kvm -boot once=d -usbdevice tablet -net nic,macaddr=%s,model=virtio -net tap,script=/var/cos/cvm/qemu-ifup,downscript=/var/cos/cvm/qemu-ifdown,ifname=VM%d -chardev socket,id=CL%d,path=%s/CL%d,server,nowait -monitor chardev:CL%d -chardev socket,id=QM%d,path=%s/QM%d,server,nowait -qmp chardev:QM%d > /dev/null 2>&1 &", id, cpu, cpu_type, mem, file, fake, virtio, isodir, os_iso, id+100, mac, id, id, cldir, id, id, id, qmdir, id, id);
//    printf("%s",cmd);
    xmlog(cmd);
    exec(cmd);	
    usleep(100000);
...
//find same name iso in data center  
bool os_same(const char* name, const char* ip) {
    char temp_name[1024];
    char *p1;
    sprintf(temp_name, "%s\n", name);
    ...
    char *p = result;
    while((p1=strstr(p, temp_name)) != NULL ) {
        if (p1 == result)
	    break;
        if ( *(p1-1) == '\n') 
	    break;
...
bool tempoff(int id) {
	...
	if(monitor(id, "system_powerdown", result,1)) {
		return true;
	} else{
		return false;
	}
...
//		scanf("%d %d %s %d %s", &cpu, &mem, mac, &type, br);
		scanf("%d %s %d %s", &cpu, cpu_type, &mem, mac);
		//scanf("%d %d  %s", &cpu, &mem, mac);
		if(vmon(id, cpu, cpu_type, mem, mac, type, br)) {
			printf("ok\n");
		} else{
			printf("error\n");
		}
...
void power_on() {
  ...
  char cpu_type[512];
	int type; char br[32];
  //scanf("%d %d %s %d %s %d %s", &id, &cpu, cpu_type, &mem, mac, &type, br);
  scanf("%d %d %s %d %s", &id, &cpu, cpu_type, &mem, mac);
  if(vmon(id, cpu, cpu_type, mem, mac, type, br)) {
    printf("ok\n");
  } else{
    printf("error\n");
...
// after migration , we need to delete the source files
 void migrate_delete1(){
   int id;
   char ip[32];
   scanf("%d%s", &id, ip);
   sprintf(cmd, "ssh root@%s rm %s/VM%d", ip, vmdir, id);
   xmlog(cmd);
   system(cmd);
   printf("ok\n");
}
...
void brctl_addbr() {
	int type, tag;
	char br[32], eth[32], ip[32], netmask[32], gateway[32], network[32];
	scanf("%d%d%s%s%s%s%s%s", &type, &tag, br, eth, ip, netmask,gateway,network);
	sprintf(cmd, "%s/addbr.sh %d %d %s %s %s %s %s %s > /dev/null 2>&1", dir, type, tag, br, eth, ip, netmask,gateway,network);
	xmlog(cmd);
	system(cmd);
	printf("ok\n");
}
...
void brctl_addif() {
	int id, type;
	char pre_br[32], cur_br[32];
	while(true) {
		scanf("%d", &id);
		if(-1 == id) break;
		scanf("%d%s%s", &type, pre_br, cur_br);
		sprintf(cmd, "%s/addif.sh %d %d %s %s > /dev/null 2>&1", dir, id, type, pre_br, cur_br);
		xmlog(cmd);
		system(cmd);
		printf("ok\n");
		fflush(stdout);
...
void brctl_delbr() {
    ...
    while(true) {
        ...
        if(-1 == tag) break;
        scanf("%s%s", br, eth);
        sprintf(cmd, "%s/delbr.sh %d %s %s > /dev/null 2>&1", dir, tag, br, eth);
        //	printf("%s\n", cmd);
        xmlog(cmd);
        system(cmd);
        printf("ok\n");
...
void diskk_expend() {
	...
	if(vmison(id)) {
		...
		sprintf(cmd, "qemu-img resize %s/VM%d +%dG > /dev/null", vmdir, id, disks);
		xmlog(cmd);
		system(cmd);
		printf("ok\n");
	}
...
void os_power_on()
{
    ...
    char cpu_type[512];
    char os_iso[1024];
    scanf("%d %d %s %d %s %s", &id, &cpu, cpu_type, &mem, mac, os_iso);
    if(oson(id, cpu, cpu_type, mem, mac, os_iso)) {
	printf("ok\n");
	vmadd(1, id, id);
    } else{
	printf("error\n");
...
void os_upload() {
    ...
    char name[1024];
    char ip[32];
    scanf("%d %s %s", &id, name, ip);
    if (os_same( name, ip)) {
        time_t t;
        time(&t);
        sprintf(cmd, "rsync -avP %s/BACK%d root@%s:%s/%s > %s/%lld &", vmdir, id, ip, datasysdir, name, logdir, t);
        xmlog(cmd);
        system(cmd);
        printf("%lld\n",t);
    } else {
...
//    char vncbuff[1024];
    int id;
    vncconf =  fopen(vnctoken, "a");
    while(true) {
	    scanf("%d", &id); 
	    if(id == -1)
	        break;
    	fprintf(vncconf, "VM%d: localhost:%d\n", id, 6000+id);
        monitor(id, "change vnc password cac028", result);
    }
    fclose(vncconf);
    printf("ok\n");
...
void delVMview() {
    ...
    while(true) {
	    ...
	    sprintf(cmd, "sed -i '/VM%d:/d' %s", id, vnctoken);
        system(cmd);
        sprintf(cmd, "change vnc password \"\"");
//        printf("%s\n",cmd);
        monitor(id, cmd, result);
    }
    printf("ok\n");
}
/*
...
void temp_create(){
	...
	char os_iso[1024];
	char mac[1024];
	scanf("%d %s %d %d %d %s %s", &id, os_name, &cpu, &mem, &disk, mac, os_iso);
//	scanf("%s",os_name);
//	scanf("%s",
//	scanf("%d", &disk);
//	scanf("%s", os_iso);
	sprintf(file, "%s/%s-OS%d", sysdir, os_name, id);
	sprintf(cmd, "qemu-img create -f qcow2 %s %dG > /dev/null", file, disk);
	xmlog(cmd);
	system(cmd);
	sprintf(cmd, "qemu-kvm -name OS%d -smp %d,maxcpus=240 -m %d -drive file=%s,if=ide,media=disk,index=0 -drive file=%s,if=virtio,media=disk,index=1 -drive file=%s,if=ide,media=cdrom -cdrom %s/%s -vnc :%d -enable-kvm -boot once=d -usbdevice tablet -net nic,macaddr=%s,model=virtio -net tap,script=/var/cos/cvm/qemu-ifup,downscript=/var/cos/cvm/qemu-ifdown,ifname=OS%d -chardev socket,id=CLOS%d,path=%s/CLOS%d,server,nowait -monitor chardev:CLOS%d -chardev socket,id=QMOS%d,path=%s/QMOS%d,server,nowait -qmp chardev:QMOS%d > /dev/null 2>&1 &", id, cpu, mem, file, fake, virtio, isodir, os_iso, id+1000, mac, id, id, cldir, id, id, id, qmdir, id, id);
	xmlog(cmd);
	exec(cmd);
	printf("ok\n");
}
...
void temp_poweron(){
	int id,cpu,mem;
	char os_name[namelen];
	char mac[namelen];
	scanf("%d %s %d %d %s", &id, os_name, &cpu, &mem, mac);	
//	if(!vmison(id,1)) {
//	sprintf(file, "%s/%s-OS%d", sysdir, os_name, id);
//	sprintf(cmd, "qemu-kvm -name OS%d -smp 2 -m 2048 -drive file=%s,if=ide,media=disk,index=0 -drive file=%s,if=virtio,media=disk,index=1 -drive file=%s,if=ide,media=cdrom -vnc :%d -enable-kvm -boot once=d -usbdevice tablet -net nic,model=virtio -chardev socket,id=CLOS%d,path=%s/CLOS%d,server,nowait -monitor chardev:CLOS%d -chardev socket,id=QMOS%d,path=%s/QMOS%d,server,nowait -qmp chardev:QMOS%d > /dev/null 2>&1 &", id, file, fake, virtio, id+1000, id, cldir, id, id, id, qmdir, id, id);	
//	xmlog(cmd);
//	exec(cmd);
//	}
	if(tempon(id, os_name, cpu, mem, mac)) {
		printf("ok\n");
	} else{
		printf("error\n");
	}	
...
/*	} else if(strcmp(cmd,"template") == 0){
		scanf("%d", &ctl);
		//template_create,template_modify,template_delete
		if(1 == ctl) {
			temp_create();
		} else if(2 == ctl) {
			temp_poweron();
...
*/
		...
		scanf("%d", &ctl);
		//add VM viewmode delete VM viewmode;
		if(1 == ctl) {
			addVMview();
		} else if(2 == ctl) {
			delVMview();
