...
进程的列表放在任务队列（task list），是一个双向循环列表，表项为`task_struct`（成为进程描述符）的结构。
...
内核区域有vm_area_struct结构体描述，它描述了指定地址空间内连续区间上的一个独立内存范围。进程的地址空间是由允许进程使用的全部线性地址组成。每个进程各不相同。内核通过所谓`线性区（也就是VMA）`来表示线性地址区间，它是由一些起始地址、长度和一些访问权限描述。
